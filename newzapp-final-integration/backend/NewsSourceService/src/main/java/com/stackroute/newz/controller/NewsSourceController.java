...
public class NewsSourceController {
	...
	@PostMapping
	public ResponseEntity<?> createNewsSource(@RequestBody NewsSource newNewsSource) {
		final boolean createNewsSourceResponse = newsSourceService.addNewsSource(newNewsSource);
		if (createNewsSourceResponse)
			return new ResponseEntity<>(createNewsSourceResponse, HttpStatus.CREATED);
		else {
	...
	public ResponseEntity<?> deleteNewsBySourceId(@PathVariable int newsSourceId) {
		final boolean response = newsSourceService.deleteNewsSource(newsSourceId);
		if (response)
	...
	public ResponseEntity<?> updateNewsSource(@PathVariable int newsSourceId, @RequestBody NewsSource updatedNewsSource) throws NewsSourceNotFoundException {
		return new ResponseEntity<>(newsSourceService.updateNewsSource(updatedNewsSource, newsSourceId), HttpStatus.OK);
	}
	...
	public ResponseEntity<?> getNewsBySourceId(@PathVariable("userId") String userId, @PathVariable("newsSourceId") int newsSourceId) throws NewsSourceNotFoundException {
		return new ResponseEntity<>(newsSourceService.getNewsSourceById(userId, newsSourceId), HttpStatus.OK);
	}
	...
	@GetMapping("/{userId}")
	public ResponseEntity<?> getNewsByUserId(@PathVariable("userId") String userId) throws NewsSourceNotFoundException {
		return new ResponseEntity<>(newsSourceService.getAllNewsSourceByUserId(userId), HttpStatus.OK);
	}

	/*
	 * Define a handler method which will block specific newssource inturn delete all news. 
	 * This handler method should return any one of the status messages basis on
	 * different situations: 
	 * 1. 200(OK) - If the newssource is blocked successfully. msg: NewsSource has been blocked successfully
	 * 2. 404(NOT FOUND) - If the newssource is not found. msg: NewsSource not found for given user
	 * This handler method should map to the URL "/block/{userId}/{newssourceId}" using HTTP GET 
	 * method.
	 * 
	 */
